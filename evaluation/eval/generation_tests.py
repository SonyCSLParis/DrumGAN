...
class StyleGEvaluationManager(object):
    ...
    def test_multi_scale_gen(self, pitch=55):
        input_z = self.ref_rand_z[:2].clone()
        if self.att_dim > 0:
            input_z[:, -self.att_dim:] = torch.zeros(self.att_dim)
        ...
        mean_z = input_z[1:2]
        gen_batch = []
        gen_batch.append(self.model.test(input_z, 
                        toCPU=True, 
                        getAvG=True,
                        mixing_range=(-1, -1)))

        for i in range(self.model.netG.nScales):
            ...
            gen_batch.append(self.model.test([z, mean_z], 
                                            ...
                                            style_weight=1,
                                            mixing_range=(i, i+1)))
            gen_batch.append(self.model.test([mean_z, z], 
                                            toCPU=True, 
                                            getAvG=True,
                                            mean_style=0,
                                            style_weight=1,
    ...
    def test_mean_weight_style(self, pitch=55):
        input_z = self.ref_rand_z.clone()
        input_z[:, -self.att_dim:] = torch.zeros(self.att_dim)
        ...
        mean_z = input_z[2:3]
        gen_batch = []
        gen_batch.append(self.model.test(input_z[:5, :], 
                                        toCPU=True, 
                                        getAvG=True,
                                        mixing_range=(-1, -1)))
        for i in linspace(0.1, 2, 20, True):

            gen_batch.append(self.model.test(z, 
                                            toCPU=True, 
                                            getAvG=True,
                                            mean_style=i,
                                            style_weight=0.5,
                                            mixing_range=(-1, -1)))
        gen_batch.append(self.model.test(mean_z, 
                                        toCPU=True, 
                                        getAvG=True,
                                        mixing_range=(-1, -1)))
        for i in linspace(0.1, 2, 10, True):   
            gen_batch.append(self.model.test(mean_z, 
                                            toCPU=True, 
                                            getAvG=True,
                                            mean_style=0.5,
    ...
    def test_full_attribute_sweep(self):
        input_z = []
        for i in range(self.att_dim):
            z = self.ref_rand_z[0].clone()
            z[-self.att_dim:] = torch.zeros(self.att_dim)
            z[-self.att_dim + i] = 1
            input_z.append(z)
