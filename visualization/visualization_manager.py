...
class LossVisualizer(TensorVisualizer):
    ...
    def publish_classification_report(self, y_true, y_pred, name, output_dir, att_list, iter):
        fig, fig_cm = plotly_classification_report(y_true, y_pred, class_list, name, output_dir)

        self.update_tokens(win)
        self.update_tokens(win_cm)
        self.window_tokens[win_cm] = \
            vis.plotlyplot(fig_cm, env=self.env + '_clf_report' , win=self.window_tokens[win_cm])
        
        self.window_tokens[win] = \
            vis.plotlyplot(fig, env=self.env + '_clf_report', win=self.window_tokens[win]) 

        if output_dir:
            plot(fig, filename=join(output_dir, win + '.html'), auto_open=False)
            plot(fig_cm, filename=join(output_dir, win_cm + '.html'), auto_open=False)
...
class AudioVisualizer(TensorVisualizer):
    ...
    def publish_waveform(self, audio, title, win, env=''):
        ...
        env += '_wave'
        self.update_tokens(win)
        fig_wave = plotlyScatter(audio, title=win + f'_{title}')
        self.window_tokens[win] = \
            vis.plotlyplot(fig_wave, env=self.env + '_' + env, win=win)
        if self.save:
            plot(fig_wave, filename=join(self.output_dir, win + f'_waveform_{title}.html'), auto_open=False)
    ...
    def publish_spectrogram(self, audio, title, win, env=''):
        ...
        if self.save:
            plot(fig_spec, filename=join(self.output_dir, win + f'_spectrum_{title}.html'), auto_open=False)

    def publish_rainbowgram(self, audio, title, win, env=''):
        win += '_rainbg'
        env += '_rainb'
        self.update_tokens(win)
        fig_rain = plotRainbowgram(audio, title=win + f'_{title}')
        self.window_tokens[win] =  \
            vis.matplot(fig_rain, 
                        ...
                        opts={'resizable': True})
        if self.save:
            save_matplot_fig(join(self.output_dir, win + f'_rainbowgram_{title}.png'))
...
class STFTVisualizer(AudioVisualizer):
    ...
    def publish_complex_spec(self, audio, title, win, env):
        ...
        self.window_tokens[win] = \
            vis.plotlyplot(fig_spec, env=self.env + '_' + env, win=win)
        if self.save:
            plot(fig_spec, filename=join(self.output_dir, win + f'_complex_{title}.html'), auto_open=False)
...
class MelVisualizer(AudioVisualizer):
    ...
    def publish_mel_spec(self, data, title, win, env):
        ...
        self.window_tokens[win] = \
            vis.plotlyplot(fig_spec, env=self.env + '_' + env, win=win)
        if self.save:
            plot(fig_spec, filename=join(self.output_dir, win + f'_mel_{title}.html'), auto_open=False)
...
class MFCCVisualizer(AudioVisualizer):
    ...
    def publish_mfcc(self, data, title, win, env):
        ...
        self.window_tokens[win] = \
            vis.plotlyplot(fig_spec, env=self.env + '_' + env, win=win)
        if self.save:
            plot(fig_spec, filename=join(self.output_dir, win + f'_mfcc_{title}.html'), auto_open=False)
